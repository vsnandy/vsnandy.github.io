...
const createDraftStore = (initialState) =>
    create((set) => ({
        ...
        addParticipant: (participant) => {
            set((state) => ({
                teams: state.teams.push({
                    id: state.teams.length + 1,
                    Username: state.teams.length + 1,
...
const Draft = ({ league, useDraftStore, schools, token }) => {
    const { teams, draftedPlayers, availablePlayers, draftPlayer, addParticipant, currentTeamIndex, rounds } = useDraftStore();
    const [search, setSearch] = useState("");
    const [timeLeft, setTimeLeft] = useState(30);
    const [isTeamsComplete, setIsTeamsComplete] = useState(false);
    ...
    const handleSubmit = (e) => {
      ...
      setFormData({ name: '', nickname: '' }); // reset form
    };

    const continueToDraft = () => {
        setIsTeamsComplete(true);
    }
        ...
        return (
            <div className="max-w-4xl mx-auto p-4 space-y-4">
                ...
                <div className="p-4 border rounded-lg bg-gray-100">
                    <h2 className="text-xl font-semibold mb-3">Setup League Participants</h2>
                    ...
                    <ul className="mt-4">
                        {teams.map((team, index) => (
                            <li key={index} className="border-b py-2">
                                <strong>{team.name}</strong> — {team.email}
                            </li>
                        ))}
                    </ul>
                </div>

                <Button variant="primary" onClick={continueToDraft}>Continue</Button>
            </div>
        );
    } else {
        return (
